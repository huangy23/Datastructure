...
int vector_operations(int &secret_code) {
  ...
  for(int i=counter-1; i >= 0; --i) { //change 26
...
int vector_sum(std::vector<int> &inVec) { // change 20
  for(uint i=1; i<inVec.size(); ++i) {//change 18
    inVec[i] = inVec[i] + inVec[i-1];
  }
  return inVec[inVec.size()-1]; //change 19
}
...
int arithmetic_operations(int &secret_code) {
  ...
  int c = 4;
  int d = c - b;              // -42
  int e = b - 3*a + 4*c;      //  32 change 7
  int f = 2*b + 2*c;          //  100
  int g = e - (b/c) + d + 20; // -1 
  int h = (f/e) ;             //  3 change 8
  int m = (d / h) / 7;        // -2 change 9
  int n = g + m;              // -3 
  int p = (f / e) - h - 1;        // -1 change 13
  int q = f + 2*d;            // 16
  int r = 2*m + p + n;        // -8 change 10
  float s = ((float)a) / f;   // 0.1 change 11
...
int main(int argc, char* argv[]) {
 ...
 int secret_code = 42;
  ...
  if(ops == "--arithmetic-operations" || ops == "--all-operations") {
    //int secret_code; //change 2 secret code
...
bool vector_compare(const std::vector<int> v1, const std::vector<int> v2) {
  ...
  bool success = true;
  for(uint i=0; i<std::min(v1.size(),v2.size()); ++i) {//change 23

    if(v1[i] <= v2[i]) {//change 24
      success = false;
...
int list_operations(int &secret_code) {
  ...
  for(std::list<int>::iterator itr = l500.begin(); itr != l500.end(); ++itr) {
    ...
    if(*itr % factor == 0 || *itr % factor2 == 0) { //change 29
  ...
  for(std::list<std::string>::reverse_iterator ritr = not_fruits.rbegin();
    ...
    fruits.erase(fruit_itr); //change 31
  ...
  int count = 0; //change 32

 for(std::list<char>::iterator itr = --l1.end(); itr != l1.begin(); itr--) { // change 33
...
int array_operations(int &secret_code) {
  ...
  for(int x=0; x<=size-1; ++x) { //change 14
  ...
  for(int x=0; x<=size-1; ++x) {   //change 15
    for(int y=0; y<=size-1; ++y) {
  ...
  int** tmp_ptr = array;
  for(int x = 1; x <= size; ++x, ++tmp_ptr) {//change 16
    int* tmp_ptr2 = *tmp_ptr;
 
 
    for(int y = 1; y <= size; ++y, ++tmp_ptr2) {//change 17
      int tmp = *tmp_ptr2;
...
float multidivide(int numerator, int d1, int d2, int d3, int d4) {
  float f = (((((float)numerator / d1) / d2) / d3) / d4); //change 12
  return f;
}
...
const std::string file_operations(int argc, char** argv, char*& returned_buffer, int& retlen) {
  ...
  if (argc != 4) {
    ...
    return "ERROR: bad arguments"; //change 3
  }
...
*/
int pythagoras(int x, int y) {
  double placeholder; // will store the integer part from modf //change 4
  ...
  if((close_enough(0,fracpart))) //change 6 == 
