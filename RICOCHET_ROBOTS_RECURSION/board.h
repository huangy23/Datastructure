...
static const std::string DIRECTION_NORTH = "north";
static const std::string DIRECTION_SOUTH = "south";
static const std::string DIRECTION_WEST = "west";
static const std::string DIRECTION_EAST = "east";

static const int MAX_MOVES = 65536;

static const int PRINT_ONE = -1;
static const int PRINT_NONE = 0;
static const int PRINT_ALL = 1;
...
public:
  ...
  // solution
  void visualize(int robot, int max_moves);
  void findSolution(bool all_solutions, int max_moves);
...
private:
  ...
  // memorized search function and representations
  void memorizedSearch(std::vector<std::pair<int, std::string>> &moves, int move, int max_moves, bool print_all);
  std::string robotPositionsToStr();
  Position getRobotPositionFromStr(const std::string &str, int robot);
  bool goalSatisfied();
  void restoreRobot(int i, Position &p);

  std::unordered_map<std::string, int> searched_moves;
